...
def query(start,stop):
    ...
    null2=4
    query_str=struct.pack('>Liiiihh',len,unknow_id,account,null1,null2,start,stop)
...
def insert_online(database,table,count):
    try:
        ...
        ret=cur.execute("insert into %s (online_num,create_time) values (%d,'%s')" %(table,count,create_time))
