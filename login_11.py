...
def main():
    ...
    params_game = urllib.urlencode(
        {'': randnum, 'username': '16156240', 'password':
         'ff81e29ca22d9d897ee18a62a443b9c6'})
    # account.game.com 验证登录过程
    opener = urllib2.build_opener()
    ret_game = opener.open("http://192.168.1.10/login/baidulogin",
                           params_game)
    data = ret_game.read()
    ...
    ret_baidu = opener_baidu.open("http://skygame.baidu.com/tp/kukuAuth.jsp",
                                  params_baidu)
    baidu_code = ret_baidu.read().split(
